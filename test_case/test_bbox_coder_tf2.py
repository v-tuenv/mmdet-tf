...
PATH =Path(os.getcwd())
sys.path.append(str(PATH.absolute()) + "/")
from mmdet.core_tf.builder import build_bbox_coder
from mmdet.core_tf.bbox.box_coders import delta_xywh_coder as faster_rcnn_box_coder 
from mmdet.core_tf.common import box_list
from mmdet.utils import test_case 
class DeltaXYWHBBoxCoderTest(test_case.TestCase):
  def test_cfg(self):
    ...
    def graph_fn(boxes, anchors):
      cfg = dict(type='DeltaXYWHBBoxCoder')
  ...
  def test_get_correct_relative_codes_after_encoding(self):
    ...
    def graph_fn(boxes, anchors):
      boxes = box_list.BoxList(boxes)
      anchors = box_list.BoxList(anchors)
      coder = faster_rcnn_box_coder.DeltaXYWHBBoxCoder()
      rel_codes = coder.encode(boxes, anchors)
      return rel_codes
    rel_codes_out = self.execute(graph_fn, [boxes, anchors])
    self.assertAllClose(rel_codes_out, expected_rel_codes, rtol=1e-04,
