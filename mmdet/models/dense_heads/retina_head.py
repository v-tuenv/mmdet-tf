...
from .anchor_head import AnchorHead, AnchorHeadV2
...
@HEADS.register_module()
class RetinaHeadV2(AnchorHeadV2):
    r"""An anchor-based head used in `RetinaNet
    <https://arxiv.org/pdf/1708.02002.pdf>`_.
    The head contains two subnetworks. The first classifies anchor boxes and
    the second regresses deltas for the anchors.
...
#         m_init_layers()
    ...
    def m_init_layers(self):
        ...
        self.reg_convs = tf.keras.Sequential(reg_convs)
        self.retina_cls =tf.keras.layers.Conv2D(self.num_anchors * self.cls_out_channels,3,padding='SAME',kernel_initializer=kernel_init, bias_initializer=tf.constant_initializer(-np.log((1 - 0.01) / 0.01)),)
        self.retina_reg = tf.keras.layers.Conv2D(self.num_anchors *4, 3 , padding='SAME',kernel_initializer=kernel_init,bias_initializer=tf.random_normal_initializer(stddev=0.01),kernel_regularizer=regularizers.l1_l2(l1=1e-5, l2=1e-4))
