...
def ConvModule(in_channels,
    ...
    with_activation = act_cfg is not None
    ...
    if with_activation:
        ...
        activate =tf.keras.layers.Activation(act_cfg_['type'].lower()) 
...
#             pad_cfg = dict(type=padding_mode)
#             # self.padding_layer = build_padding_layer(pad_cfg, padding)
#             self.padding_layer = tf.keras.layers.ZeroPadding2D(padding=(padding, padding))
...
#             super(ConvModule, self).__setattr__(self.norm_name, norm)
...
#             self.activate =tf.keras.layers.Activation(act_cfg_['type'].lower()) 
