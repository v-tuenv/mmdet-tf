...
class Match(object):
  ...
  def __init__(self, match_results, use_matmul_gather=False):
    ...
    if use_matmul_gather:
      self._gather_op = matmul_gather_on_zeroth_axis
...
class Matcher(six.with_metaclass(abc.ABCMeta, object)):
  ...
  def match(self,groundtruth_boxes, anchors, valid_rows=None, scope=None):
    ...
    """
    similarity_matrix = self._similarity_calc.compare(groundtruth_boxes, anchors)
