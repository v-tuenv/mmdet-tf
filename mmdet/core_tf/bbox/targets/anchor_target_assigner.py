...
from mmdet.core_tf.bbox.box_coders import delta_xywh_coder
from mmdet.core_tf.bbox.box_coders import base_coder
from mmdet.core_tf.common import box_list, standart_fields
from mmdet.core_tf.common import shape_utils
from mmdet.core_tf.common import standart_fields as fields
from mmdet.core_tf.anchors.anchor_generator import AnchorGenerator
...
from mmdet.core_tf.builder import TARGET_ASSIGNER
@TARGET_ASSIGNER.register_module()
class TargetAnchorAssignerV2(object):
    def __init__(self,
        ...
        self.keep_static_anchor = keep_static_anchor
        if self.keep_static_anchor is False:
            assert isinstance(anchor_generator,AnchorGenerator)
        self.anchor_generator=anchor_generator
    @property
    def box_coder(self):
        return self._box_coder
    def to_levels(self, value, num_levels):
        level_targets = []
        start = 0
        for i in range(len(num_levels)):
            ...
            level_targets.append(value[ start:end,...])
            start = end
        return level_targets

    def __call__(self,value):   
        boxes = box_list.BoxList(value[fields.InputDataFields.groundtruth_boxes])
        classes = tf.cast(value[fields.InputDataFields.groundtruth_classes],tf.int32)
        gt_classes = tf2.where(tf.reshape(classes,(-1,)) >= 0,1.,0.)
        # gt_classes=tf.reshape(gt_classes,(-1,))
        # gt_classes = tf.cast(gt_classes,tf.float32)
        anchors_list = self.anchor_generator
        num_level_anchors = [anchors.shape[0] for anchors in anchors_list]
        concat_anchor = box_list.BoxList(tf.concat(anchors_list, axis=0))
        (cls_targets, cls_weights, reg_targets, reg_weights,
                match_results) = self.assign(
                    concat_anchor,
                    boxes,
                    groundtruth_labels=classes,
                    groundtruth_weights=gt_classes
                )
        num_positive_fields=tf.reduce_sum(reg_weights)
        cls_targets = self.to_levels(cls_targets,num_level_anchors)
        cls_weights = self.to_levels(cls_weights, num_level_anchors)
        reg_targets = self.to_levels(reg_targets, num_level_anchors)
        reg_weights = self.to_levels(reg_weights, num_level_anchors)
        for index,i in enumerate(num_level_anchors):
            value[fields.TargetComputeFields.regression_target+f"_{index}"] = reg_targets[index]
            value[fields.TargetComputeFields.regression_weight+f"_{index}"] = reg_weights[index]
            value[fields.TargetComputeFields.classification_target+f"_{index}"] = cls_targets[index]
            value[fields.TargetComputeFields.classification_weight+f"_{index}"] = cls_weights[index]
        value[standart_fields.TargetComputeFields.num_positive_fields] = num_positive_fields
        return value
    def assign(self,
             anchors,
             groundtruth_boxes,
        ...
        with tf.control_dependencies(
            ...
            match = self._matcher.match(groundtruth_boxes,anchors,
                                        valid_rows=tf.greater(groundtruth_weights, 0))
            match = self.sampler.sampler(match)
